...
const upload = multer({
  dest: UPLOAD_DIR,
  limits: { fileSize: 25 * 1024 * 1024 }, // 25MB (Whisper上限)
...
// クライアント初期化（APIキーはリクエスト毎に受け取る）
function getOpenAI(apiKey) {
  return new OpenAI({ apiKey });
}
function getClaude(apiKey) {
  return new Anthropic({ apiKey });
}

// ────────────────────────────────────────
// POST /api/transcribe  音声→テキスト変換
// ────────────────────────────────────────
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    if (!transcript) {
      return res.status(400).json({ error: "文字起こしテキストが必要です" });
    }
    ...
    const diagramInstruction = settings?.includeDiagrams
      ? `\n\n## 図表について\n内容に応じて、以下の形式でMermaid図表を1〜3個挿入してください：\n\`\`\`mermaid\n（図表コード）\n\`\`\`\n適切な図表タイプ：flowchart（フロー）、mindmap（マインドマップ）、sequenceDiagram（シーケンス）、classDiagram（クラス図）`
      : "";
...
## 出力フォーマット
JSONで以下の形式で返答してください（\`\`\`jsonで囲まず、純粋なJSONのみ）：
{
  "title": "記事タイトル（魅力的かつ簡潔に）",
  "summary": "3〜5文の要約",
  "tags": ["タグ1", "タグ2", "タグ3", "タグ4", "タグ5"],
  "body": "Markdown形式の本文（見出し・リスト・図表を含む）",
...
${transcript}`;
    ...
    // JSON抽出（```json フェンスがある場合も対応）
    let parsed;
    try {
      const jsonMatch = rawText.match(/```json\s*([\s\S]*?)```/) || rawText.match(/({[\s\S]*})/);
      parsed = JSON.parse(jsonMatch ? jsonMatch[1] : rawText);
    } catch {
      // JSONパース失敗時はテキストをbodyとして返す
      parsed = {
        title: "生成された記事",
        summary: "",
        tags: [],
        body: rawText,
        readingTime: Math.ceil(rawText.length / 400)
      };
    }
...
function buildToneInstructions(settings) {
  ...
  if (settings.tone) {
    const tones = {
      formal: "です・ます調の丁寧な文体を使用してください。",
      casual: "フランクで親しみやすい口語的な文体を使用してください。",
      academic: "客観的で論理的な学術的文体を使用してください。",
      energetic: "熱量高くエネルギッシュな文体で読者を引き込んでください。",
      gentle: "穏やかで優しい文体を使用してください。"
    };
    lines.push(tones[settings.tone] || settings.tone);
  }

  if (settings.ending) {
    const endings = {
      desu_masu: "文末は「です」「ます」で統一してください。",
      da_dearu: "文末は「だ」「である」で統一してください。",
      casual_yo: "文末に「よ」「ね」「よね」を適度に使って親しみやすくしてください。",
      question: "読者に語りかけるような疑問形を適度に交えてください。"
    };
    lines.push(endings[settings.ending] || `語尾は「${settings.ending}」を使用してください。`);
  }

  if (settings.targetAudience) {
    lines.push(`対象読者：${settings.targetAudience}`);
  }

  if (settings.articleLength) {
    const lengths = {
      short: "800〜1200文字程度の短めの記事にしてください。",
      medium: "1500〜2500文字程度の標準的な長さにしてください。",
      long: "3000文字以上の詳細な記事にしてください。"
    };
    lines.push(lengths[settings.articleLength] || "");
  }

  if (settings.customInstruction) {
    lines.push(`追加指示：${settings.customInstruction}`);
  }
...
// ────────────────────────────────────────
// POST /api/regenerate-section  セクション再生成
// ────────────────────────────────────────
app.post("/api/regenerate-section", async (req, res) => {
  ...
  try {
    ...
    const message = await claude.messages.create({
      ...
      messages: [{
        role: "user",
        content: `以下のテキストを指示に従って書き直してください。\n\n指示：${instruction}\n\n元のテキスト：\n${section}\n\n書き直したテキストのみを返してください。`
      }]
    });
...
app.listen(PORT, () => {
  console.log(`🎙️ MP3記事生成アプリ起動 → http://localhost:${PORT}`);
